...
class PlatformGenerator {
  constructor(terrainImage, TILE_SIZE = 16) {
    this.terrainImage = terrainImage;

    this.TILE_SIZE = TILE_SIZE;
    this.TILE_SPACING = 15.4;
  ...
  createHorizontalPlatform(startX, y, numTiles) {
    ...
    tiles.push({
      ...
      cropbox: this.TILES.PLAT_LEFT,
    });

    for (let i = 1; i < numTiles - 1; i++) {
      tiles.push({
        position: { x: startX + this.TILE_SPACING * i, y: y },
        cropbox: this.TILES.PLAT_CENTER,
      });
    }

    if (numTiles > 1) {
      tiles.push({
        position: { x: startX + this.TILE_SPACING * (numTiles - 1), y: y },
        cropbox: this.TILES.PLAT_RIGHT,
      });
    } else if (numTiles === 1) {
      tiles[0].cropbox = this.TILES.PLAT_CENTER;
    }

    return tiles;
  }

  createFloatingPlatforms(startX, platformsConfigs = []) {
    const platforms = [];

    platformsConfigs.forEach((config) => {
